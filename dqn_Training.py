...
agent = DQN_AGENT(EPSILON, action_space, q_func, DEVICE)
buffer = ReplayBuffer(int(episodes*episode_len/2), BATCH_SIZE, DEVICE, action_space)
...
for n_episode in range(N_EPIODES):
    ...
    while not D_t:                            
        ...
        S_t = S_prime
        if self_play:
            A_t_2 = torch.abs(A_t-torch.tensor([1]).to(DEVICE))
        ...
        if n_steps > WARM_START and n_steps % TRAINING_FREQUENCY == 0:
            ...
            agent.train(b_s, b_a, b_r, b_d, b_s_, dqn_type=dqn_type)
...
model_path = os.path.join(os.getcwd(), model_q_func_name)
